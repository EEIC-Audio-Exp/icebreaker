...
while true; do
	...
	julius -C asr/grammar.jconf -filelist ${tmpdirname}/list.txt 2> /dev/null | grep "^cmscore1: " > ${cmscore}
	...
	./response.py dialogue/dialogue${sidnum}.conf $sidnum $asrresult $cmscore

	# 事後処理
	rm $filename $sidfile $asrresult $cmscore
done
# ここは実行されないはず
rmdir $tmpdirname
